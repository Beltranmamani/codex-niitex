...
<head>
    ...
    <title><?= SISTEMA_NOMBRE; ?> - Entregas pendientes </title>
...
<body class="alt-menu sidebar-noneoverflow">
                        ...
                        <table class="table table-bordered table-striped mb-4">
                            ...
                            <tfoot>
                                ...
                                <tr>
                                    ...
                                    <th class='text-left' id="descuento_detalle">0</th>
                                </tr>
                                <tr>
    ...
    <div id="modal_mapa" class="modal animated slideInUp custo-slideInUp" role="dialog" data-backdrop="static">
        <div class="modal-dialog ">
            <!-- Modal content-->
            <div class="modal-content" id='loader_detail'>
                <div class="modal-header">
                    <h5 class="modal-title">MAPA DE UBICACION</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x">
                            <line x1="18" y1="6" x2="6" y2="18"></line>
                            <line x1="6" y1="6" x2="18" y2="18"></line>
    ...
    <script>
        ...
        mymap.on('locationfound', (e) => {
            ...
            var greenIcon = new LeafIcon({
                iconUrl: "<?= SERVERURL; ?>view/distribucion/camion.png"
            });
        ...
        let mytable = function() {
            ...
            $.post("<?= SERVERURL; ?>preventa/lista_preventas_pendientes/", {
                sucursal
            }, function(response) {
                if (response == 0) {
                    Snackbar.show({
                        text: 'No hay preventas disponibles',
                        actionTextColor: '#fff',
                    ...
                    c2 = $('#table_compras').DataTable({
                        ...
                        buttons: {
                            buttons: [{
                                ...
                                extend: 'pdfHtml5',
                                title: "REPORTE DE PREVENTAS - <?= $_SESSION["nombre_sucursal"] ?> - <?= date('Y-m-d H:i:s') ?>",
                                className: 'btn',
    ...
    <script>
        ...
        $(document).on('click', '.btn_detalle', function() {
            ...
            $.post("<?= SERVERURL ?>preventa/detallepreventa/", {
                id
            }, function(response) {
                if (response == 0) {
                    Snackbar.show({
                    ...
                    if (resd[0] == 1) {
                        ...
                        $("#total_detalle").text(resd[14]);
                        $.post("<?= SERVERURL ?>preventa/detallepreventa_item/", {
                            id
                        }, function(respuesta) {
                            if (response != 0) {
        ...
        $(document).on('click', '.btn_mapa', function() {
            // var mymap = L.map('mapa').setView([-16.49948, -68.14477], 13);
            ...
            let l1 = $(this).attr('l1');
            let l2 = $(this).attr('l2');
            let n_preventa = $(this).attr('n_preventa');
            let direccion_fisica = $(this).attr('direccion_fisica');
            let destino = $(this).attr('destino');
            setTimeout(function() {
                L.marker([l1, l2]).addTo(mymap)
                    .bindPopup("<b>DESTINARIO: " + destino + " </b> <br />Direccion:" + direccion_fisica + " <br />Preventa: " + n_preventa + " <br />").openPopup();
        ...
        $("#btn_consultar").click(function() {
            ...
            if (fecha_1.length < 1 || fecha_2.length < 1) {
                ...
                if (fecha_2 < fecha_1 || fecha_1 > fecha_2) {
                    ...
                    $.post("<?= SERVERURL; ?>preventa/lista_preventa_fecha_pendientes/", {
                        fecha_1,
                        fecha_2,
                        sucursal
                    }, function(response) {
                        if (response == 0) {
                            Snackbar.show({
                                text: 'No hay preventas disponibles en ese rango de fechas',
                                actionTextColor: '#fff',
                                backgroundColor: '#e7515a',
                                pos: 'top-right'
                            });
                            ...
                            c2 = $('#table_compras').DataTable({
                                ...
                                buttons: {
                                    buttons: [{
                                        ...
                                        extend: 'pdfHtml5',
                                        title: "REPORTE DE PRVENTA - <?= $_SESSION["nombre_sucursal"] ?> - <?= date('Y-m-d H:i:s') ?>",
                                        className: 'btn',
                                        orientation: 'landscape',
                                        pageSize: 'A4',
                                        exportOptions: {
        ...
        $(document).on('click', '.btn_anular', function() {
            ...
            let preventa_estado = $(this).attr('preventa_estado');
            if (preventa_estado == 1) {
                ...
                swalWithBootstrapButtons({
                    title: 'Estas seguro?',
                    text: "Es irrevertible el cambio si anulas la preventa!",
                    type: 'warning',
                    ...
                    if (result.value) {
                        $.post("<?= SERVERURL ?>preventa/anular_preventa/", {
                            id_preventa
                        }, function(response) {
                            ...
                            if (response == 1) {
                                toast({
                                    type: 'success',
                                    title: 'Preventa anulada correctamente',
                                    padding: '2em',
                                });
                                mytable();
                            } else {
                ...
                toast({
                    type: 'error',
                    title: 'Lo siento, Ya es una venta realizada',
                    padding: '2em',
                });
                ...
                toast({
                    type: 'error',
                    title: 'Lo siento, preventa ya anulada',
                    padding: '2em',
                })
            }
        });
