...
<head>
    ...
    <title><?=SISTEMA_NOMBRE;?> - Cuentas por pagar</title>
...
<body class="alt-menu sidebar-noneoverflow">
    ...
    <script>
        $(document).ready(function(){
            $("#view_compras, #vista_credito_fecha").addClass("active");
            ...
            c2 = $('#table_compras').DataTable({
                ...
                buttons: {
                    buttons: [
                        { 
                            ...
                            extend: 'pdfHtml5', 
                            title : "REPORTE DE CREDITOS POR FECHA - <?=$_SESSION["nombre_sucursal"]?> - <?=date('Y-m-d H:i:s')?>",
                            className: 'btn',
                ...
                "oLanguage": {
                    ...
                    "sZeroRecords": "Seleccione un rango de fechas...",
        ...
        let cuentas = () =>{
            ...
            $.post("<?=SERVERURL;?>compras/reporte_cuentas_pagar_lista/",{sucursal},function(response){            
                if(response == 0){
                    Snackbar.show({
                        text: 'No hay creditos disponibles',
                        actionTextColor: '#fff',
        ...
        $(document).on('click','#btn_consultar',function(){
            ...
            if(fecha_1.length <1 || fecha_2.length <1){
                ...
                if(fecha_2<fecha_1 || fecha_1>fecha_2){
                    ...
                    $.post("<?=SERVERURL;?>compras/reporte_cuentas_pagar/",{sucursal,fecha_1,fecha_2},function(response){            
                        if(response == 0){
                            Snackbar.show({
                                text: 'No hay creditos disponibles',
                                actionTextColor: '#fff',
...
/* ========================================================================== */
/*                     funcion consultar compras por fecha                    */
/* ========================================================================== */
        $(document).on('click','.btn_detalle',function(){
            ...
            $.post("<?=SERVERURL?>compras/detallecompra/",{id},function(response){
                if(response == 0){
                    Snackbar.show({
                    ...
                    if(resd[0]== 1){
                        ...
                        $.post("<?=SERVERURL?>compras/detallecompra_item/",{id},function(respuesta){
                            if(response != 0 ){
