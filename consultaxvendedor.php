...
<head>
    ...
    <title><?=SISTEMA_NOMBRE;?> - Preventas por vendedor</title>
...
<body class="alt-menu sidebar-noneoverflow">
    ...
    <script>
        $(document).ready(function(){
            $("#view_preventa, #vista_prventaxvendedor,#dp_reporte_preventa").addClass("active");
            ...
            if(vendedor == null){
                ...
                if(fecha_1.length <1 || fecha_2.length <1){
                    ...
                    if(fecha_2<fecha_1 || fecha_1>fecha_2){
                        ...
                        $.post("<?=SERVERURL;?>preventa/lista_preventa_vendedor/",{vendedor,sucursal,fecha_1,fecha_2},function(response){            
                            if(response == 0 || response == ""){
                            Snackbar.show({
                                text: 'No hay productos disponibles en ese rango de fechas',
                                actionTextColor: '#fff',
                                ...
                                buttons: {
                                    buttons: [
                                        { 
                                            ...
                                            extend: 'pdfHtml5', 
                                            title : "REPORTE DE PRODUCTOS PREVENDIDOS - <?=$_SESSION["nombre_sucursal"]?> - <?=date('Y-m-d H:i:s')?>",
                                            className: 'btn',
                                            orientation :'landscape',
                                            pageSize:'A4',
                                            exportOptions: {
...
/* ========================================================================== */
        ...
        $(document).on('click','.btn_detalle',function(){
            ...
            $.post("<?=SERVERURL?>preventa/detallepreventa/",{id},function(response){
                if(response == 0){
                    Snackbar.show({
                    ...
                    if(resd[0]== 1){
                        ...
                        $.post("<?=SERVERURL?>preventa/detallepreventa_item/",{id},function(respuesta){
                            if(response != 0 ){
